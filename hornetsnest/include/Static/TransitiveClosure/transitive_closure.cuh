...
//Modified July 26, 2020
template <bool countOnly>
struct OPERATOR_AntisectionCountBalanced {
    trans_t* d_CountNewEdges;
    vid_t* d_src ;
    vid_t* d_dest;
...
// Modified July 14, 2020
        ...
        //         printf("*%d\n",vertex.neighbor_ptr()[i]);
		    ...
		    printf("Coundonly find duplicated edge %d->[%d-1]=%d,[%d]=%d\n",vertex.id(),i,vertex.neighbor_ptr()[i-1],i,vertex.neighbor_ptr()[i]);
                }else{
                    trans_t pos = atomicAdd(newBatchSize,1);
                    srcDelete[pos] = vertex.id();
                    destDelete[pos] = vertex.neighbor_ptr()[i];
		    printf("Memory allocation find duplicated edge %d->[%d-1]=%d,[%d]=%d\n",vertex.id(),i,vertex.neighbor_ptr()[i-1],i,vertex.neighbor_ptr()[i]);
                }
            }
        }
    }
